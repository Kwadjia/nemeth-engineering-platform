import {
  PROTOTYPE_STATUSES,
  type BuildAction,
  type BuildRecordRead,
  type PartInstanceRead,
  type PrototypeRead,
...
import { InstanceStatusBadge, PrototypeStatusBadge } from "@/components/domain/prototypeBadges";
...
function ConfigurationPanel({ prototypeRef }: { prototypeRef: string }) {
  const config = usePrototypeConfiguration(prototypeRef);
  return (
    <Card>
      <CardHeader
...
function BuildLogPanel({ prototypeRef }: { prototypeRef: string }) {
  const builds = useBuildRecords(prototypeRef);
  return (
    <Card>
      ...
      {builds.isLoading ? (
        ...
        <CardContent>
          <EmptyState
            ...
            description="The first build record starts this prototype's history."
            className="py-6"
          />
        </CardContent>
      )}
...
function NewBuildForm({ prototype }: { prototype: PrototypeRead }) {
  const create = useCreateBuildRecord(prototype.identifier);
  const available = usePartInstances({ status: "AVAILABLE" });
  const removed = usePartInstances({ status: "REMOVED" });
  const installed = usePartInstances({ prototype_id: prototype.id });
...
export function InstanceStatusCell({ status }: { status: PartInstanceRead["status"] }) {
  return <InstanceStatusBadge status={status} />;
}
